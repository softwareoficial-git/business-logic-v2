...
class DataEngine {
    ...
    async getNamespace(namespace) {
        const res = await this.dbPool.query('SELECT data FROM public.cliente_data_sheets WHERE cliente_id = $1 AND namespace = $2', [this.tenantId, namespace]);
        return res.rows.length > 0 ? res.rows[0].data : { meta: { next_field_id: 1, next_value_id: 1 } };
    }
    async saveNamespace(namespace, data) {
        const query = `
    ...
    async createField(label, parentFieldId) {
        const data = await this.getNamespace('dynamic_catalog');
        if (!data.meta)
            data.meta = { next_field_id: 1, next_value_id: 1 };
    ...
    async createValue(fieldId, value, parentId) {
        const data = await this.getNamespace('dynamic_catalog');
        if (!data.meta)
            data.meta = { next_field_id: 1, next_value_id: 1 };
    ...
    async resolveFieldId(fieldLabel) {
        const data = await this.getNamespace('dynamic_catalog');
        const field = Object.entries(data.fields || {}).find(([id, f]) => f.label === fieldLabel);
        return field ? field[0] : undefined;
    }
    async ensureValueId(fieldLabel, valueLabel, parentValueLabel) {
        const data = await this.getNamespace('dynamic_catalog');
        ...
        let parentId;
        if (parentValueLabel) {
            const parentFieldEntry = Object.entries(data.fields || {}).find(([id, f]) => f.label === fieldLabel && f.parent_field_id);
            if (parentFieldEntry) {
                const parentFieldIdFromEntry = parentFieldEntry[1].parent_field_id;
                const parentFieldLabel = data.fields[parentFieldIdFromEntry]?.label;
                const resolvedParentFieldId = await this.resolveFieldId(parentFieldLabel);
                const parentValueEntry = Object.entries(data.values || {}).find(([id, v]) => resolvedParentFieldId && v.field_id === resolvedParentFieldId && v.value === parentValueLabel);
                if (!parentValueEntry)
                    throw new Error(`Parent value "${parentValueLabel}" not found for field "${parentFieldLabel}".`);
                parentId = parentValueEntry[0];
            }
        }
        const existingValue = Object.entries(data.values || {}).find(([id, v]) => v.field_id === fieldId && v.value === valueLabel && (parentId ? v.parent_id === parentId : !v.parent_id));
        if (existingValue) {
    ...
    // --- PRODUCTOS Y STOCK ---
    async getProductFullData(productCode) {
