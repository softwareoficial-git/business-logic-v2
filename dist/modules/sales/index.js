...
class SalesModule {
    ...
    async checkout(context, params) {
        ...
        await InfraClient_1.infraClient.pushItem(context.tenantId, 'sales', saleRecord, context.token);
    ...
    async createOrder(context, params) {
        ...
        if (client_request_id) {
            const res = await InfraClient_1.infraClient.queryJson(context.tenantId, 'sales', { client_request_id }, context.token);
        ...
        const saleRes = await InfraClient_1.infraClient.pushItem(context.tenantId, 'sales', saleRecord, context.token);
        if (!saleRes.success)
            return saleRes;
        ...
        const ordersRes = await InfraClient_1.infraClient.readPath(context.tenantId, 'sales', context.token);
        if (ordersRes.success && Array.isArray(ordersRes.data)) {
            const orders = ordersRes.data;
            const idx = orders.findIndex(o => o.id === saleId);
            if (idx !== -1) {
                await InfraClient_1.infraClient.updatePath(context.tenantId, `sales[${idx}].payment_link`, paymentLink, context.token);
            }
        }
        return {
            success: true,
    ...
    async getSummary(context, params) {
        const ordersRes = await InfraClient_1.infraClient.readPath(context.tenantId, 'sales', context.token);
    ...
    async getHistory(context, params) {
        const res = await InfraClient_1.infraClient.readPath(context.tenantId, 'sales', context.token);
        if (!res.success && res.error?.code === 'PATH_NOT_FOUND') {
            return { success: true, message: 'No sales history found', data: [] };
        }
        return res;
    }
    async getMonthlySummary(context, params) {
    ...
    async getDailyTotal(context, params) {
